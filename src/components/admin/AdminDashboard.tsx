...
export function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  useEffect(() => {
    ...
    const messagesSub = subscribeToTable('messages', () => {
      loadAnalytics();
  ...
  const loadCurrentViewData = async () => {
    ...
    try {
      switch (currentView) {
        case 'projects':
          setData({ projects: await storage.getProjects() });
          break;
        case 'posts':
          setData({ posts: await storage.getPosts() });
          break;
        case 'videos':
          setData({ videos: await storage.getVideos() });
          break;
        case 'certificates':
          setData({ certificates: await storage.getCertificates() });
          break;
        case 'jobs':
          setData({ jobs: await storage.getJobs() });
          break;
        case 'reviews':
          setData({ reviews: await storage.getReviews() });
          break;
        case 'qa':
          setData({ qas: await storage.getQAs() });
          break;
        case 'messages':
          setData({ messages: await storage.getMessages() });
          break;
        case 'newsletter':
          setData({ newsletter: await storage.getNewsletterSubscriptions() });
          break;
      }
    } catch (error) {
